...
@router.get("/physical_data/{user_id}", response_model=PhysicalDataResponse)
def get_physical_data(user_id: int, db: Session = Depends(get_db)):
...
@router.put("/update/{user_id}", response_model=PhysicalDataResponse)
def update_physical_data(user_id: int, data: PhysicalDataCreate, db: Session = Depends(get_db)):
    physical_data = db.query(PhysicalData).filter(PhysicalData.user_id == user_id).first()
    if not physical_data:
        raise HTTPException(status_code=404, detail="Physical data not found")
